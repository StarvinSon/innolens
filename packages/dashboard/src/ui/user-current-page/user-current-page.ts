...
const predictions: SpaceCountPrediction = (() => {
  const records: Array<SpaceCountPredictionRecord> = [];
  let currentTime = startOfHour(new Date());

  for (let i = 0; i < (24 - getHours(new Date())); i += 1) {
    records.push({
      startTime: currentTime,
      endTime: addHours(currentTime, 1),
      counts: {
        'Electrical and Electronic Engineering': Math.cos(i * 15) + 2,
        'Mechanical Engineering': Math.sin(i * 15 + 5) + 2,
        'Computer Science': Math.sin(i * 15 + 10) + 3
      }
    });
...
export class UserCurrentPage extends LitElement {
  ...
  private async _updateProperties(): Promise<void> {
    ...
    if (!this._dataFetched) {
      ...
      this._countPrediction = predictions;
