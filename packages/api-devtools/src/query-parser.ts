...
export const writeQueryDecoder = (
  ...
  if ('anyOf' in schema) {
    ...
    switch (schema.type) {
      case 'null': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryNull');
        writer.writeLine(`${tempTargetVar} = ${parseMethod}(${tempSrcVar})`);
        break;
      }
      case 'boolean': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryBoolean');
        writer.writeLine(`${tempTargetVar} = ${parseMethod}(${tempSrcVar})`);
        break;
      }
      case 'integer': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryInteger');
        writer.writeLine(`${tempTargetVar} = ${parseMethod}(${tempSrcVar})`);
        break;
      }
      case 'number': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryNumber');
        writer.writeLine(`${tempTargetVar} = ${parseMethod}(${tempSrcVar})`);
        break;
      }
      case 'string': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryString');
        writer.writeLine(`${tempTargetVar} = ${parseMethod}(${tempSrcVar})`);
        break;
      }
      case 'date': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryDate');
        writer.writeLine(`${tempTargetVar} = ${parseMethod}(${tempSrcVar})`);
        break;
      }
      case 'array': {
        const parseMethod = imports.addNamedImport(`@innolens/api-runtime/lib-${runtimeType}/query-parser`, 'decodeQueryArray');
