...
export const writeJsonEncoder = (
  ...
  if ('anyOf' in schema) {
    ...
    switch (schema.type) {
      case 'null': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonNull');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'boolean': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonBoolean');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'integer': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonInteger');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'number': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonNumber');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'string': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonString');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'date': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonDate');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'array': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonArray');
        ...
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar}, (${itemVar}${addLambdaParamUnknownType ? ': unknown' : ''}) => `).inlineBlock(() => {
          ...
          writeJsonEncoder(
            sourceFile, writer, createVarName, imports,
            encodedItemVar, itemVar, schema.items,
            runtimeType, addLambdaParamUnknownType
          );
      ...
      case 'object': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-stringifier`, 'encodeJsonObject');
