...
export const writeJsonDecoder = (
  ...
  if ('anyOf' in schema) {
    ...
    switch (schema.type) {
      case 'null': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonNull');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'boolean': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonBoolean');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'integer': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonInteger');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'number': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonNumber');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'string': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonString');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'date': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonDate');
        writer.writeLine(`${resultVar} = ${encodeFunc}(${srcVar})`);
        break;
      }
      case 'array': {
      ...
      case 'object': {
        const encodeFunc = imports.addNamedImport(`@innolens/api/runtime/${runtimeType}/json-parser`, 'decodeJsonObject');
